...
expCtx.font='54px "Share Tech Mono"';expCtx.textBaseline="top";expCtx.fillStyle=l;expCtx.fillText(d.text+": "+c.text.hex,36,h+54);h+=k}},mediaTiny=window.matchMedia("(max-width: 900px)"),mediaSmall=window.matchMedia("(min-width: 1300px)");mediaTiny.addListener(function(b){b.matches?(expCtx.clearRect(0,0,exportCanvas.width,exportCanvas.height),drawSwatchesMobile(currentExportObject)):(expCtx.clearRect(0,0,exportCanvas.width,exportCanvas.height),drawSwatches(currentExportObject))});
mediaSmall.addListener(function(b){b.matches&&(expCtx.clearRect(0,0,exportCanvas.width,exportCanvas.height),drawSwatches(currentExportObject))});
