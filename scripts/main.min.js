...
c.indexOf(b)){var d=b;randomParamsStart.value=c[c.indexOf(d)];randomParamsEnd.value=c[c.indexOf(a)];b=a;a=d}t({start:a,end:b})};randomParamsStart.addEventListener("input",m);randomParamsEnd.addEventListener("input",m);void 0!=STORAGE.getItem("darkMode")&&"false"==STORAGE.getItem("darkMode")?(globalTheme="light",themeSelect.selectedIndex=1,BODY.classList.remove("dark")):(globalTheme="dark",themeSelect.selectedIndex=0,BODY.classList.add("dark"));themeSelect.addEventListener("input",function(){"dark"==
themeSelect.value&&0==BODY.classList.contains("dark")&&BODY.classList.add("dark");"light"==themeSelect.value&&1==BODY.classList.contains("dark")&&BODY.classList.remove("dark");BODY.classList.contains("dark")?STORAGE.setItem("darkMode","true"):STORAGE.setItem("darkMode","false")});var u=function(){for(var a="#",b=0;6>b;b++)a+="0123456789ABCDEF"[Math.floor(16*Math.random())];return a},q=function(){if(bgLocked&&textLocked)return!1;0<colorHistory.length&&1==initialRandomize&&(initialRandomize=!1,Array.from(document.getElementsByClassName("control-button")).forEach(function(a){return a.classList.remove("disabled")}));
var c=!0===bgLocked?colorObject.bg.hex:u(),d=!0===textLocked?colorObject.text.hex:u(),h=hexToRGBA(c),l=hexToRGBA(d),g="rgb("+h.r+", "+h.g+", "+h.b+")",v="rgb("+l.r+", "+l.g+", "+l.b+")";a.red.value=h.r;a.green.value=h.g;a.blue.value=h.b;b.red.value=l.r;b.green.value=l.g;b.blue.value=l.b;if(1==contrastRatiosSet){var c=getContrastRatio(c,d).number,d=[k,r],m=["fail","aa large","aa","aaa"],n;for(n in wcagLevels)for(var p in wcagLevels[n])if(h=wcagLevels[n][p],c>=h[0]&&c<=h[1])if(l=[].concat($jscomp.arrayFromIterable(m)),
d=Array.from(new Set([].concat($jscomp.arrayFromIterable(d)))),h=l.indexOf(d[0]),l=-1!==l.indexOf(d[1])?l.indexOf(d[1]):h,h=function(a,b){return Array(b-a+1).fill(void 0).map(function(b,c){return c+a})}(h,l).map(function(a){return m[a]}),n=n.toLowerCase(),0==h.includes(n))try{q()}catch(B){throw CURRENT_ERR="there is no acceptable color combination for the current settings. please modify contrast ratio limit.",new RangeError(CURRENT_ERR);}else if(h.includes(n)){f(g,!1);e(v,!1);setComputedColors(!0);
break}}else f(g,!1),e(v,!1),setComputedColors(!0)};document.body.onkeydown=function(a){32==a.keyCode&&document.activeElement==document.body&&a.preventDefault()};document.body.onkeyup=function(a){if(32==a.keyCode&&document.activeElement==document.body){a.preventDefault();if(document.getElementById("randomizeReminder")){var b=document.getElementById("randomizeReminder");b.classList.add("hidden");setTimeout(function(){return b.remove()},300)}q()}};document.getElementById("randomize-mobile").addEventListener("click",
function(){return q()});for(var g in a)a.hasOwnProperty(g)&&a[g].addEventListener("input",function(){f("rgb("+a.red.value+", "+a.green.value+", "+a.blue.value+")")});for(var w in b)b.hasOwnProperty(w)&&b[w].addEventListener("input",function(){e("rgb("+b.red.value+", "+b.green.value+", "+b.blue.value+")")});swapColors.addEventListener("click",function(){a.red.value=colorObject.text.rgb.r;a.green.value=colorObject.text.rgb.g;a.blue.value=colorObject.text.rgb.b;b.red.value=colorObject.bg.rgb.r;b.green.value=
