...
$jscomp.polyfill("Array.from",function(a){return a?a:function(a,c,d){$jscomp.initSymbolIterator();c=null!=c?c:function(a){return a};var b=[],f=a[Symbol.iterator];if("function"==typeof f)for(a=f.call(a);!(f=a.next()).done;)b.push(c.call(d,f.value));else for(var f=a.length,h=0;h<f;h++)b.push(c.call(d,a[h]));return b}},"es6-impl","es3");$jscomp.array=$jscomp.array||{};
...
var paletteControls=function(a,b,c){currentPaletteIndex=c;currentPaletteObject=a;switch(b){case "remove":a.classList.add("hide");setTimeout(function(){paletteHumanIndex--;currentPalette--;PALETTES.splice(c,1);0==PALETTES.length&&(PALETTES=[[]]);paletteTotalCount=PALETTES.length;0>currentPalette&&(currentPalette=0);savePaletteState();return a.remove()},500);break;case "export":return exportPalette(a,c)}},copyContent=function(a,b,c){a=void 0===a?null:a;c=void 0===c?!0:c;null!=a&&a.preventDefault();
window.getSelection().selectAllChildren(b);document.execCommand("copy");c&&showCopyNotification();window.getSelection().removeAllRanges()},setPigmentTextColor=function(a){Array.from(a.children).forEach(function(a){var b=a.style.backgroundColor.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/),d=b[2],e=b[3],b=Math.round((299*parseInt(b[1])+587*parseInt(d)+114*parseInt(e))/1E3);125<b&&0==a.classList.contains("blackText")?a.classList.add("blackText"):125>b&&1==a.classList.contains("blackText")&&a.classList.remove("blackText")})},
loadSavedPalettes=function(){PALETTES.forEach(function(a){var b=paletteBarTemplate.cloneNode(!0);b.id="";b.classList.add("palette-bar");b.classList.remove("empty");b.getElementsByClassName("palette-number")[0].textContent="palette "+paletteHumanIndex;var c=b.getElementsByClassName("palette-number")[0].textContent;paletteHumanIndex++;var d=b.getElementsByClassName("export-palette")[0],e=b.getElementsByClassName("remove-palette")[0],f={name:c,palette:b};paletteContainer.appendChild(b);e.addEventListener("click",
function(){return paletteControls(b,"remove",Array.from(paletteContainer.children).indexOf(b))});d.addEventListener("click",function(){return paletteControls(f,"export",Array.from(paletteContainer.children).indexOf(b))});a.forEach(function(a){var c=palettePigmentTemplate.cloneNode(!0);c.children[0].style.backgroundColor=a.bg.hex;c.children[1].style.backgroundColor=a.text.hex;c.children[0].textContent=a.bg.hex;c.children[1].textContent=a.text.hex;setPigmentTextColor(c);c.children[0].addEventListener("click",
function(a){return copyContent(a,c.children[0])});c.children[1].addEventListener("click",function(a){return copyContent(a,c.children[1])});c.id="";c.classList.add("palette-pigment");b.appendChild(c)})})};PALETTES&&0<PALETTES.length&&0!=PALETTES[0].length&&loadSavedPalettes();
...
palette:d};paletteHumanIndex++;1==c&&addPigment(d,a,b,!0);a=d.getElementsByClassName("export-palette")[0];d.getElementsByClassName("remove-palette")[0].addEventListener("click",function(){paletteControls(d,"remove",Array.from(paletteContainer.children).indexOf(d))});a.addEventListener("click",function(){return paletteControls(e,"export",Array.from(paletteContainer.children).indexOf(d))});paletteTotalCount=PALETTES.length},addPigment=function(a,b,c,d){d=void 0===d?!1:d;var e=palettePigmentTemplate.cloneNode(!0);
e.children[0].style.backgroundColor=b;e.children[1].style.backgroundColor=c;e.children[0].textContent=b;e.children[1].textContent=c;var f=hexToRGBA(b),h=hexToRGBA(c);1==d&&(b={bg:{hex:b,rgb:f},text:{hex:c,rgb:h}},0!=PALETTES[currentPalette].length&&(currentPalette++,PALETTES.push([])),PALETTES[currentPalette].push(b),savePaletteState());setPigmentTextColor(e);e.children[0].addEventListener("click",function(a){return copyContent(a,e.children[0])});e.children[1].addEventListener("click",function(a){return copyContent(a,
e.children[1])});e.id="";e.classList.add("palette-pigment");a.appendChild(e)},addColorToPalette=function(a,b){palettePigmentTemplate.cloneNode(!0).id="";var c=rgbToHex(a.r,a.g,a.b),d=rgbToHex(b.r,b.g,b.b),e=paletteContainer.children[paletteTotalCount-1],f={bg:{hex:c,rgb:a},text:{hex:d,rgb:b}};5>PALETTES[currentPalette].length?(PALETTES[currentPalette].push(f),addPigment(e,c,d)):addNewPaletteBar(c,d);savePaletteState()};
...
var makeFullPalette=function(){var a=paletteGenParams.count,b=hexToRGBA(colorObject.bg.hex),c=hexToRGBA(colorObject.text.hex),d=[],e=[];d.push(b);e.push(c);if("gradient"==paletteGenParams.setting){addNewPaletteBar(colorObject.bg.hex,colorObject.bg.hex);b=function(a,b,c){var d=function(a,b,c){c||(c=.5);for(var d=a.slice(),e=0;3>e;e++)d[e]=Math.round(d[e]+c*(b[e]-a[e]));return d},e=1/(c-1),f=[];a=[].concat($jscomp.arrayFromIterable(Object.values(a)));b=[].concat($jscomp.arrayFromIterable(Object.values(b)));
for(var g=0;g<c;g++)f.push(d(a,b,e*g));return f}(d[0],e[0],paletteGenParams.count);for(c=1;c<b.length;c++){var f={r:b[c][0],g:b[c][1],b:b[c][2]};d.push({r:b[c][0],g:b[c][1],b:b[c][2]});e.push(f)}d.forEach(function(a,b){0<b&&addColorToPalette(a,e[b])})}if("monochromatic"==paletteGenParams.setting){addNewPaletteBar(colorObject.bg.hex,colorObject.bg.hex);for(b=1;b<a;b++)c=shiftSat(d[b-1],-paletteGenParams.factor),f=shiftSat(d[b-1],-paletteGenParams.factor),d.push(c),e.push(f);d.forEach(function(a,b){0<
b&&addColorToPalette(a,e[b])})}if("analogous"==paletteGenParams.setting){addNewPaletteBar(colorObject.bg.hex,colorObject.text.hex);for(b=1;b<a;b++)c=shiftHue(d[b-1],paletteGenParams.factor),f=shiftHue(e[b-1],paletteGenParams.factor),d.push(c),e.push(f);d.forEach(function(a,b){0<b&&addColorToPalette(a,e[b])})}savePaletteState()};document.getElementById("generate-palette").addEventListener("click",function(){return makeFullPalette()});
