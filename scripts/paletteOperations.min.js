...
d){var e=toCamelCase(a.colorNames[d].bg),q=c.bg.hex,g=toCamelCase(a.colorNames[d].text);b.push({bg:{name:e,hex:q},text:{name:g,hex:c.text.hex}})});for(var d=[],e=[],f=0;f<b.length;f++){var h=b[f].text.name,p=b[f].text.hex;d.push({name:b[f].bg.name,hex:b[f].bg.hex});e.push({name:h,hex:p})}var g=d.concat(e);g.sort(propSort("name"));var k=0;g.forEach(function(a,b){0<b&&g[b-1].name==a.name?(k++,g[b].name+="-"+k):0<b&&g[b-1].name==a.name+("-"+k)?(k++,g[b].name+="-"+k):k=0});var l="",m="";"sass"==c&&g.forEach(function(a,
...
b.appendChild(c)})})};PALETTES&&0<PALETTES.length&&0!=PALETTES[0].length&&loadSavedPalettes();
...
palette:d};paletteHumanIndex++;1==b&&addPigment(d,a,c,!0);a=d.getElementsByClassName("export-palette")[0];d.getElementsByClassName("remove-palette")[0].addEventListener("click",function(){return paletteControls(d,"remove",paletteTotalCount-1)});a.addEventListener("click",function(){return paletteControls(e,"export",paletteTotalCount-1)});0<PALETTES[0].length&&paletteTotalCount++},addPigment=function(a,c,b,d){d=void 0===d?!1:d;var e=palettePigmentTemplate.cloneNode(!0);e.children[0].style.backgroundColor=
c;e.children[1].style.backgroundColor=b;e.children[0].textContent=c;e.children[1].textContent=b;var f=hexToRGBA(c),h=hexToRGBA(b);1==d&&(c={bg:{hex:c,rgb:f},text:{hex:b,rgb:h}},currentPalette++,PALETTES.push([]),PALETTES[currentPalette].push(c),savePaletteState());setPigmentTextColor(e);e.children[0].addEventListener("click",function(a){return copyContent(a,e.children[0])});e.children[1].addEventListener("click",function(a){return copyContent(a,e.children[1])});e.id="";e.classList.add("palette-pigment");
a.appendChild(e)},addColorToPalette=function(a,c){palettePigmentTemplate.cloneNode(!0).id="";var b=rgbToHex(a.r,a.g,a.b),d=rgbToHex(c.r,c.g,c.b),e=paletteContainer.children[paletteTotalCount-1],f={bg:{hex:b,rgb:a},text:{hex:d,rgb:c}};5>PALETTES[currentPalette].length||(currentPalette++,PALETTES.push([]));PALETTES[currentPalette].push(f);savePaletteState();6>e.children.length?addPigment(e,b,d):addNewPaletteBar(b,d)};
