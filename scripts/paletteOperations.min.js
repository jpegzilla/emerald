...
1==mobilecheck()?drawSwatchesMobile(a):drawSwatches(a):drawSwatches(a);switch(exportType){case "sass":exportTextContainer.children[0].classList.add("active");break;case "css":exportTextContainer.children[1].classList.add("active");break;case "image":exportTextContainer.children[2].classList.add("active")}},exportPalette=function(a,b,c){(void 0===c||c)&&openExportModal();c=PALETTES[b];b=!1;0==FANCY_COLOR_NAMES&&(b=FANCY_COLOR_NAMES=!0,setColorNames());for(var d=[],e=0;e<c.length;e++)d.push({bg:findNearestColor(c[e].bg.hex),
text:findNearestColor(c[e].text.hex)});a={colorNames:d,name:a.name,palette:c};b||(FANCY_COLOR_NAMES=!1,setColorNames());makeExportContent(a)};
...
document.getElementById("export-canvas").toDataURL();(new Image).src=a;var c=document.createElement("a");c.href=a;c.download="emerald-palette.png";c.click()}});
var paletteControls=function(a,b,c){currentPaletteIndex=c;currentPaletteObject=a;switch(b){case "remove":a.classList.add("hide");setTimeout(function(){paletteHumanIndex--;paletteTotalCount--;currentPalette--;PALETTES.splice(c,1);savePaletteState();return a.remove()},500);break;case "export":return exportPalette(a,c)}},copyContent=function(a,b,c){a=void 0===a?null:a;c=void 0===c?!0:c;null!=a&&a.preventDefault();window.getSelection().selectAllChildren(b);document.execCommand("copy");c&&showCopyNotification();
window.getSelection().removeAllRanges()},setPigmentTextColor=function(a){Array.from(a.children).forEach(function(a){var b=a.style.backgroundColor.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/),d=b[2],e=b[3],b=Math.round((299*parseInt(b[1])+587*parseInt(d)+114*parseInt(e))/1E3);125<b&&0==a.classList.contains("blackText")?a.classList.add("blackText"):125>b&&1==a.classList.contains("blackText")&&a.classList.remove("blackText")})},loadSavedPalettes=function(){PALETTES.forEach(function(a){var b=paletteBarTemplate.cloneNode(!0);
b.id="";b.classList.add("palette-bar");b.classList.remove("empty");b.getElementsByClassName("palette-number")[0].textContent="palette "+paletteHumanIndex;var c=b.getElementsByClassName("palette-number")[0].textContent;paletteHumanIndex++;var d=b.getElementsByClassName("export-palette")[0],e=b.getElementsByClassName("remove-palette")[0],f={name:c,palette:b},h=Array.from(paletteContainer.children).indexOf(b);e.addEventListener("click",function(){return paletteControls(b,"remove",h)});d.addEventListener("click",
function(){return paletteControls(f,"export",h)});paletteContainer.appendChild(b);a.forEach(function(a){var c=palettePigmentTemplate.cloneNode(!0);c.children[0].style.backgroundColor=a.bg.hex;c.children[1].style.backgroundColor=a.text.hex;c.children[0].textContent=a.bg.hex;c.children[1].textContent=a.text.hex;setPigmentTextColor(c);c.children[0].addEventListener("click",function(a){return copyContent(a,c.children[0])});c.children[1].addEventListener("click",function(a){return copyContent(a,c.children[1])});
c.id="";c.classList.add("palette-pigment");b.appendChild(c)})})};PALETTES&&0<PALETTES.length&&0!=PALETTES[0].length&&loadSavedPalettes();
...
palette:d};paletteHumanIndex++;1==c&&addPigment(d,a,b,!0);a=d.getElementsByClassName("export-palette")[0];b=d.getElementsByClassName("remove-palette")[0];var f=Array.from(paletteContainer.children).indexOf(d);b.addEventListener("click",function(){return paletteControls(d,"remove",f)});a.addEventListener("click",function(){return paletteControls(e,"export",f)});0<PALETTES[0].length&&paletteTotalCount++},addPigment=function(a,b,c,d){d=void 0===d?!1:d;var e=palettePigmentTemplate.cloneNode(!0);e.children[0].style.backgroundColor=
b;e.children[1].style.backgroundColor=c;e.children[0].textContent=b;e.children[1].textContent=c;var f=hexToRGBA(b),h=hexToRGBA(c);1==d&&(b={bg:{hex:b,rgb:f},text:{hex:c,rgb:h}},currentPalette++,PALETTES.push([]),PALETTES[currentPalette].push(b),savePaletteState());setPigmentTextColor(e);e.children[0].addEventListener("click",function(a){return copyContent(a,e.children[0])});e.children[1].addEventListener("click",function(a){return copyContent(a,e.children[1])});e.id="";e.classList.add("palette-pigment");
a.appendChild(e)},addColorToPalette=function(a,b){palettePigmentTemplate.cloneNode(!0).id="";var c=rgbToHex(a.r,a.g,a.b),d=rgbToHex(b.r,b.g,b.b),e=paletteContainer.children[paletteTotalCount-1],f={bg:{hex:c,rgb:a},text:{hex:d,rgb:b}};5>PALETTES[currentPalette].length||(currentPalette++,PALETTES.push([]));PALETTES[currentPalette].push(f);savePaletteState();6>e.children.length?addPigment(e,c,d):addNewPaletteBar(c,d)};
